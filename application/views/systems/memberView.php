		...
		<div class="vertical-box">
			<div class="vertical-box-column width-300">
				...
				<div class="wrapper">
					<div class="panel panel-profile">
						<div class="panel-body">
							...
							<ul class="list-group no-margin text-left p-t-10">
								...
								<li class="list-group-item"><span class="text-info p-l-10">완료 스테이지</span><span class="badge badge-warning pull-right"><?=$arrResult->iCnt06?></span></li>
					...
					<div class="text-left">
						<a href="/systems/memberList<?=$sParam02?>" class="btn btn-primary btn-xs"><i class="fa fa-list"></i> Go List</a>
						<!-- 고객센터 회원상세로 연결 -->
					...
					<div class="panel panel-profile">
						<div class="wrapper">
							<h4 class="m-b-15 m-t-0 p-b-10 underline"><i class="fa fa-info-circle m-r-5"></i> 나눔/플러스 회원 정보
								...
								<a href="/systems/memberList<?=$sParam02?>" class="btn btn-primary btn-xs pull-right"><i class="fa fa-list"></i> Go List</a>
							</h4>
							...
							<div class="panel-body panel-form form-horizontal form-bordered">
								...
								<? } ?>
								<form data-parsley-validate="true" class="form-horizontal form-bordered" id="actForm" action="/systems/memberModifyProc" method="POST">
								<div class="alert alert-info p-10 m-b-0">
									<strong>추가정보</strong>
								</div>
								...
								<? } ?>
								<? if ($arrResult->UserGrade == 4) { ?>
								<div class="alert alert-info p-10 m-b-0">
									<strong>제휴회원</strong>
								</div>
								<div class="form-group">
									<label class="control-label col-md-3 col-sm-3" for="Contents">스테이지 개설수</label>
									<div class="col-md-8 col-sm-6 text-left">
										<input class="form-control width-125" type="text" id="StageCreateMax" name="StageCreateMax" value="<?=$arrResult->StageCreateMax?>"/>
									</div>
								</div>
								<? } ?>
								<div class="alert alert-info p-10 m-b-0">
								...
								<div class="form-group">
									<div class="col-md-12 text-center p-t-20">
										...
										<a href="/systems/memberList<?=$sParam02?>" class="btn btn-warning btn-xs"><i class="fa fa-ban"></i> Cancel</a>
									</div>
								</div>
								<? } ?>
								</form>
...
$('.Succession').click(function(e){
	$.ajax({
	url:"/systems/memberSuccessionChange?UserSuccessionYn="+$(this).data("yn")+"&Idx=<?=$arrResult->Idx?>",
	dataType:"json",
	}).done(function (data) {
		alert(data.sMessage);
		if (data.sRetCode=="01") {
