        ...
        <div class="table-responsive">
            ...
            <table class="table table-bordered table-hover table-td-valign-middle">
                ...
                <tbody>
					...
					<tr>
                       <td class="text-center" rowspan="10">SNE</td>
					   <td class="text-center" colspan="2">초기값(최초에는 합계에서 빠짐)</td>
                       <td class="text-center">2%</td>
                       <td class="text-center text-danger f-s-14 f-w-700">0.014%</td>
                       <td></td>
                       <td></td>
                    </tr>
                    ...
                    <tr>
                       <td>재매칭 횟수</td>
                       <td><input type="number" class="icss-sne-array-1" min="0" max="30" /></td>
                       <td class="text-center" class="icss-sne-ratio" data-value="2" rowspan="2">2%</td>
                       <td class="text-center text-danger f-s-13 icss-sne-array-result" rowspan="2"></td>
                       <td></td>
                       <td></td>
                    </tr>
                    <tr>
                       <td>신규 스테이지내 친구수</td>
                       <td><input type="number" class="icss-sne-array-2" min="0" max="12" /></td>
                       <td></td>
                       <td></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                       <td class="text-center" colspan="2">SNE 소계</td>
                       <td class="text-center">2%</td>
                       <td id="icss-sne-last-result" class="text-center text-danger f-s-14 f-w-700">0.000%</td>
                       <td></td>
                       <td></td>
                    </tr>
...
$(".icss-opr").change(function() {
    ...
    if (selOpr2 > selOpr1) selOpr2 = selOpr1;
    $("#icss-opr-2").val(selOpr2);
...
$(".icss-sne-array-1, .icss-sne-array-2").change(function() {

	var selSneRatio = 0.02;
	var sneLength = $('.icss-sne-array-1').length;
	var arrSneResult = [];
    var firstSneResult = 0.0138;
	var lastSneReult = 0.014;

	for (i=0; i<sneLength; i++) {
		var sneRelations = $('.icss-sne-array-1').eq(i).val();
		var sneFriends = $('.icss-sne-array-2').eq(i).val();

		if (sneRelations > 0  && sneFriends > 0) {

			if (sneRelations == '') sneRelations = 0;
			if (sneFriends == '') sneFriends = 0;
			if (sneRelations > 30) sneRelations = 30;
			if (sneFriends > 10) sneFriends = 10;

			sneRelations = parseInt(sneRelations);
			sneFriends = parseInt(sneFriends);
			...
			$.ajax({
				...
				success: function(data) {
					for(var idx=0; idx<data.length; idx++) {
						console.log(parseInt(data[idx][0]) + ' : ' + sneRelations);
						if (parseInt(data[idx][0]) == sneFriends) {
							var returnVal = data[idx][sneRelations];
							arrSneResult[i] = returnVal;
							break;
						} else {
							arrSneResult[i] = 0;
						}
					 ...
					 // 결과값을 뿌려줌
					 var returnLastVal = firstSneResult - (returnVal * selSneRatio);
                     console.log('returnLastVal : '+ returnLastVal);

					 lastSneReult -=  returnLastVal

					 $('.icss-sne-array-result').eq(i).html(parseFloat(returnLastVal).toFixed(3)+"%");
			...
			$('.icss-sne-array-result').eq(i).html("0.000%");
		}

		$('#icss-sne-last-result').html(parseFloat(lastSneReult).toFixed(3)+"%");
		$("#icss-sne-val").val(lastSneReult);

		console.log("icss-sne: "+lastSneReult);
