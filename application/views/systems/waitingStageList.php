...
<div id="content" class="content">
	...
	<!-- end breadcrumb -->
	<!-- begin page-header -->
	<h1 class="page-header">대기(추천) 스테이지 관리 <small>List</small></h1>
	<!-- end page-header -->
			...
			<table class="table table-bordered table-hover table-td-valign-middle" style="width: 1750px">
				...
				<tbody>
					...
					<tr>
						...
						<td class="text-center"><a href="/systems/waitingStageView<?=$sParam?>&Idx=<?=$row["Idx"]?>"><?=$row["StageCode"]?></a></td>
						<td class="text-center"><?=fnStageSecret($row["SecretYN"])?></td>
						<td class="text-center"><?=$row["CategoryName"]?></td>
						<td class="text-center"><a href="/systems/waitingStageView<?=$sParam?>&Idx=<?=$row["Idx"]?>"><?=$row["Title"]?></a></td>
						...
						<td class="text-center">
							<div class="btn-group">
								<a class="btn btn-info btn-xs" href="javascript:;" data-toggle="dropdown" aria-expanded="false">
									<i class="fa fa-cog"></i> 추천변경
									<i class="fa fa-angle-down"></i>
								</a>
								<ul class="dropdown-menu">
									<? if ($row["RecommendYN"]=="Y") {?>
									<li><a href="#" class="goAccept recommendYN" data-idx="<?=$row["Idx"]?>" data-recommendyn="N"> 추천해제로 변경</a></li>
									<? } else {?>
									<li><a href="#" class="goAccept recommendYN" data-idx="<?=$row["Idx"]?>" data-recommendyn="Y"> 추천등록으로 변경</a></li>
									<? } ?>
								</ul>
							</div>
							<? if ($iCnt01<2) { ?>
...
$('.recommendYN').click(function(e){
	$.ajax({
	url:"/systems/recommendYNProc?Idx="+$(this).data("idx")+"&RecommendYN="+$(this).data("recommendyn"),
	dataType:"json",
	}).done(function (data) {
		alert(data.sMessage);
		if (data.sRetCode=="01") {
