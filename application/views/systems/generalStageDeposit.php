	...
	<div class="profile-container">
		<div class="vertical-box">
					...
					<div class="panel panel-profile">
						<div class="panel-body text-left">
							...
							<ul class="nav nav-pills nav-stacked nav-sm">
								...
								<li class="active"><a href="/systems/generalStageDeposit<?=$sParam02?>&Idx=<?=$Idx?>"><i class="fa fa-sign-in fa-fw m-r-5"></i> 입금 현황 </a></li>
			...
			<div class="vertical-box-column tab-content">
				<!-- begin deposit -->
				<div id="deposit" class="p-0">
					<div class="panel panel-profile">
						<div class="wrapper">
							<h4 class="m-b-15 m-t-0 p-b-10 underline"><i class="fa fa-info-circle m-r-5"></i> 스테이지 입금 현황
								<a href="/systems/generalStageList<?=$sParam02?>" class="btn btn-primary btn-xs pull-right"><i class="fa fa-list"></i> Go List</a>
							</h4>
							<div class="tabbable-panel">
								<div class="tabbable-line">
									<ul class="nav nav-tabs bg-white f-s-15 text-center">
										<? for ($iCnt=1;$iCnt<=$StageNum;$iCnt++) {
											$sCssStyle="";
											if ($iCnt==1) { $sCssStyle="class='active'"; }
										?>
										<li <?=$sCssStyle?>><a href="#deposit-tab-<?=$iCnt?>" data-toggle="tab"><?=$iCnt?>회</a></li>
										<? } ?>
									</ul>
									<!-- 입금 현황 DB 에서 AJAX 형태로 불러오는게 좋을것 같음 -->
									<div class="tab-content">
										<?
										$iCnt=1;
										foreach ($arrResult02 as $row) {
											$sCssStyle="";
											if ($iCnt==1) { $sCssStyle="active"; }
										?>
										<div class="tab-pane <?=$sCssStyle?>" id="deposit-tab-<?=$iCnt?>">
											<div class="table-responsive">
												<table class="table table-bordered table-hover table-td-valign-middle table-primary">
													<thead>
														<tr>
															<th width="4%" class="text-center">회차</th>
															...
															<th width="7%" class="text-center">입금예정일</th>
															<th width="7%" class="text-center">입금예정액</th>
															<th width="7%" class="text-center">입금액</th>
															<th width="7%" class="text-center">조정입금액</th>
															<th width="5%" class="text-center">입금상태</th>
...
//														var_dump($row02);
														?>
														<tr>
															<td class="text-center"><?=$row02["TurnNo"]?></td>
															<td class="text-center"><?=$row02["StageCode"]?></td>
															<td class="text-center"><?=$row02["UserCode"]?></td>
															<td class="text-center"><?=$row02["MyTurn"]?></td>
															<td class="text-center"><?=$row02["UserNickName"]?></td>
															<td class="text-center"><?=$row02["ScheduledDepositDate"]?></td>
															<td class="text-center"><?=number_format($row02["ScheduledDepositMoney"])?></td>
															<td class="text-center"><?=number_format($row02["DepositMoney"])?></td>
															<td class="text-center"><?=number_format($row02["ControlDepositMoney"])?></td>
															<td class="text-center"><?=fnDepositState($row02["DepositState"])?></td>
															<td class="text-center"><?=fnStageState($row02["State"])?></td>
														</tr>
														<? } ?>
													</tbody>
												</table>
