...
<div id="content" class="content">
	...
	<!-- end breadcrumb -->
	<!-- begin page-header -->
	<h1 class="page-header">미완성 스테이지 관리 <small>List</small></h1>
	<!-- end page-header -->
			...
			<table class="table table-bordered table-hover table-td-valign-middle" style="width: 1750px">
				...
				<tbody>
					...
					<tr>
						...
						<td class="text-center"><a href="/systems/incompleteStageView<?=$sParam?>&Idx=<?=$row["Idx"]?>"><?=$row["StageCode"]?></a></td>
						<td class="text-center"><?=fnStageSecret($row["SecretYN"])?></td>
						<td class="text-center"><?=$row["CategoryName"]?></td>
						<td class="text-center"><a href="/systems/incompleteStageView<?=$sParam?>&Idx=<?=$row["Idx"]?>"><?=$row["Title"]?></a></td>
...
$('.delProc').click(function(e){
	$.ajax({
	url:"/systems/incompleteStageDelProc?Idx=" +$("#delIdx").val()+"&DelReason="+encodeURIComponent($("#DelReason").val()),
	dataType:"json",
	}).done(function (data) {
		alert(data.sMessage);
		if (data.sRetCode=="01") {
